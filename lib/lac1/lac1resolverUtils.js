...
export function getVerificationMethod( did, algo, encoding, value, controller ) {
	...
	switch( encoding ) {
		...
		case 'pem':
			...
			break
		case 'json':
			verificationMethod.publicKeyJwk = JSON.parse((Buffer.from( value.slice(2), 'hex' )).toString());
			break;
	}
	return verificationMethod;
}

export function processValidVerificationMethod(did, event, verificationMethods, services, relationships, value, key) {
	...
	switch( type ) {
		case 'vm':
			verificationMethods[key] = getVerificationMethod( did, algo, encoding, value, controller );
			break
		case 'auth':
		case 'asse':
		case 'keya':
...
export function handleVerificationMethodRemoval(relationships, verificationMethods, services) {
...
export function wrapDidDocument( did, controller, history, mode ) { // TODO: remove useless 'id' param
	...
	for( const event of history ) {
		...
		if( validTo && validTo.gte( now ) ) {
			if( event.name == 'DIDAttributeChanged' ) {
				processValidVerificationMethod(did, event, verificationMethods, services, relationships, value, key)
			...
			if( event.name == 'DIDAttributeChanged' ) {
				handleVerificationMethodRemoval(relationships, verificationMethods, services);
			} else if( event.name == "AKAChanged") {
				const id = event.args.akaId;
