...
describe( 'DID', () => {
	...
	it( 'should fail to create a DID without address', async() => {
		try {
			new DID( {
				...
				registry: '0xa41ECf4CD4d9d602ADB13bE8DD3128D816083992',
	...
	it( 'should change the DID controller using a signed tx', async() => {
		await did.changeControllerSigned( controller1.privateKey, controller2.address );
		const controller = await did.getController();

		expect( controller.toLowerCase() ).to.equals( controller2.address.toLowerCase() );
	} );

	it( 'should return to the original controller', async() => {
		const originalKey = did.config.controllerPrivateKey;
		did.setControllerKey( controller2.privateKey );
		await did.changeController( did.address );
		const controller = await did.getController();
		did.setControllerKey( originalKey );
